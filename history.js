...
/* ═══════════════════════════════════════════
   7. UPDATE TOTALS (summary strip only)
═══════════════════════════════════════════ */
...
function applyTxnFilter() {
  ...
  if (from && to && from > to) {
    if (errEl) {
      errEl.textContent = T.err_date_range || "Start date cannot be after end date";
      errEl.style.display = "block";
    }
    return;
  }
...
/* ═══════════════════════════════════════════
   19. NAVIGATION (within history page)
═══════════════════════════════════════════ */
function goToTransactions() {
  ...
  const target = $("page-transactions");
  if (target) {
...
function wire() {
  ...
  $("txnSubmit")?.addEventListener("click", () => {
    ...
    const catName =
      T[catKey] || catKey || (type === "income" ? T.cat_other_income : T.cat_other_expense);
